...
#define MAX_STR 255
	...
	while (cur_dev) {
		...
		if (cur_dev->vendor_id == VID && cur_dev->product_id == PID)
		{
			...
			printf("Found PIC at %04hx %04hx %s\n", VID, PID, cur_dev->path);
			//if (!handle)
			//	printf("Could not open the PIC\n");
		}
...
int sendPIC(hid_device *handle, unsigned char *buf)
{
	...
	for (__uint16_t i = 0; i < buf[1]; i++)
	{
		crc = (crc +  buf[i]) &0xFF;
		printf("%02hx ", buf[i]);
	}
	crc = (crc ^ 0xFF) + 1;
	buf[buf[1]] = static_cast<unsigned char>(crc & 0xFF); // CRC_L;
	printf("%02hx %04hx\n", buf[buf[1]], crc);

	return hid_write(handle, buf, buf[1]+1);
}
